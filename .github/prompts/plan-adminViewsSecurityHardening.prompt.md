...
Nouveau fichier `scripts/check-admin-views-owner.ts` (~60 lignes) vérifiant que toutes les vues `*_admin` + `*_dashboard` appartiennent à `admin_views_owner`.
...
| Créer | `scripts/check-admin-views-owner.ts` | ~60 |
...
3. **Script validation post-migration** — `check-admin-views-owner.ts` pour vérifier ownership après déploiement.
