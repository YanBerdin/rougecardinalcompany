...
export async function createUserAction(input: unknown): Promise<ActionResult> {
  ...
  return {
    success: true,
    data: result.data,
    ...(!emailSent && { warning: "User created but welcome email could not be sent" }),
  };
}
```
 ...
 */
export async function createUser(input: UserInput): Promise<DALResult<UserDTO>> {
  try {
    // 1. Authorization
    await requireAdmin();
...
export async function createFeature(input: FeatureInput): Promise<DALResult<Feature>> {
  await requireAdmin();
