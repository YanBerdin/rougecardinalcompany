...
-- Solution: SECURITY DEFINER function throws error for non-admins (proper access control)
...
-- Security: Revoke from public roles (defense in depth)
revoke all on function public.communiques_presse_dashboard() from anon, authenticated;

-- Security: Grant to service_role for admin backend
