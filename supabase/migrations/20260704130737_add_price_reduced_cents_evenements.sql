-- Migration: Add price_reduced_cents to evenements
-- Purpose: Support an independent reduced ticket price (in cents), displayed
--          alongside the full price (price_cents) in admin and on the public
--          site (/agenda + /spectacles/[slug]).
-- Affected: public.evenements (new column), public.restore_content_version
--           (restore logic updated to include the new field).
-- Special: No constraint between price_cents and price_reduced_cents (independent fields).

alter table public.evenements
  add column if not exists price_reduced_cents integer null;

comment on column public.evenements.price_reduced_cents is 'Tarif réduit en centimes, indépendant du plein tarif';

-- Recreate restore_content_version to restore price_reduced_cents alongside
-- the other evenement fields when reverting to a previous content version.
