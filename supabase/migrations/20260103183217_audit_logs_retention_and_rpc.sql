alter table "public"."medias" drop constraint "medias_folder_id_fkey";
...
alter table "public"."logs_audit" add column "expires_at" timestamp with time zone default (now() + '90 days'::interval);

alter table "public"."medias" drop column "folder_id";

CREATE INDEX idx_audit_logs_expires_at ON public.logs_audit USING btree (expires_at) WHERE (expires_at IS NOT NULL);
...
CREATE OR REPLACE FUNCTION public.get_audit_logs_with_email(p_action text DEFAULT NULL::text, p_table_name text DEFAULT NULL::text, p_user_id uuid DEFAULT NULL::uuid, p_date_from timestamp with time zone DEFAULT NULL::timestamp with time zone, p_date_to timestamp with time zone DEFAULT NULL::timestamp with time zone, p_search text DEFAULT NULL::text, p_page integer DEFAULT 1, p_limit integer DEFAULT 50)
 RETURNS TABLE(id bigint, user_id uuid, user_email text, action text, table_name text, record_id text, old_values jsonb, new_values jsonb, ip_address inet, user_agent text, created_at timestamp with time zone, expires_at timestamp with time zone, total_count bigint)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
AS $function$
