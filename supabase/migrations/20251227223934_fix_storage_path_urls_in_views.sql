...
drop policy "Admin can manage media folders" on "public"."media_folders";

drop policy "Authenticated users can view media folders" on "public"."media_folders";

drop policy "Admin can manage media item tags" on "public"."media_item_tags";

drop policy "Authenticated users can view media item tags" on "public"."media_item_tags";

drop policy "Admin can manage media tags" on "public"."media_tags";

drop policy "Authenticated users can view media tags" on "public"."media_tags";

-- Removed references to spectacles_backup_20251209120000 (table does not exist in remote)

alter table "public"."medias" drop constraint if exists "medias_folder_id_fkey";
...
-- NOTE: idx_spectacles_public was removed but will be recreated later in this migration

-- NOTE: spectacles_backup table already removed above

alter table "public"."media_folders" disable row level security;

alter table "public"."media_item_tags" disable row level security;

alter table "public"."media_tags" disable row level security;

-- NOTE: folder_id column kept (added by migration 20251227203314_add_media_tags_folders.sql)

alter table "public"."spectacles" alter column "status" set default 'draft'::text;

alter table "public"."spectacles" alter column "status" set not null;

CREATE INDEX IF NOT EXISTS idx_spectacles_search_vector ON public.spectacles USING gin (search_vector);

CREATE INDEX IF NOT EXISTS idx_spectacles_slug ON public.spectacles USING btree (slug);

CREATE INDEX IF NOT EXISTS idx_spectacles_status ON public.spectacles USING btree (status);

CREATE INDEX IF NOT EXISTS idx_spectacles_public ON public.spectacles USING btree (public);
