...
test.describe('Parcours Editor — Permissions', () => {
    ...
    test('ROLE-E2E-002 — Editor sidebar filtrée (8 items visibles)', async ({ editorPage: page }) => {
        ...
        for (const item of EDITOR_SIDEBAR_ITEMS) {
            await expect(sidebar.getByRole('listitem', { name: item })).toBeVisible();
        }

        // 3. Verify total sidebar menu-item count matches editor items
        const menuItems = sidebar.locator('[data-sidebar="content"] [data-sidebar="menu-item"]');
        await expect(menuItems).toHaveCount(EDITOR_SIDEBAR_ITEMS.length);
    });
    ...
    test('ROLE-E2E-003 — Editor sidebar — items admin masqués', async ({ editorPage: page }) => {
        ...
        for (const item of ADMIN_ONLY_SIDEBAR_ITEMS) {
            await expect(sidebar.getByRole('listitem', { name: item })).toBeHidden();
        }
    });
...
test.describe('Parcours Admin — Permissions', () => {
    ...
    test('ROLE-E2E-012 — Admin sidebar complète (18 items)', async ({ adminPage: page }) => {
        ...
        // 2. Verify total sidebar menu-item count matches all items
        const sidebar = page.locator('[data-sidebar="sidebar"]');
        const menuItems = sidebar.locator('[data-sidebar="content"] [data-sidebar="menu-item"]');
        await expect(menuItems).toHaveCount(TOTAL_ADMIN_SIDEBAR_ITEMS);
...
test.describe('API Admin — Permissions', () => {
    // ROLE-E2E-021: API backoffice — Editor authorized (200)
    // Editor has minimum role required by withBackofficeAuth (requireMinRole("editor"))
    test('ROLE-E2E-021 — API backoffice — Editor autorisé (200)', async ({ editorPage: page }) => {
        // 1. Fetch backoffice API with editor session cookies
        const response = await page.request.fetch(API_ADMIN_ENDPOINT, {
            method: 'GET',
        });

        // 2. Verify 200 OK response (editor meets minimum role)
