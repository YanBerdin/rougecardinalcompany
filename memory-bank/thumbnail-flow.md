...
>
> ⚠️ **Bug n°3 (corrigé le 2026-07-20)** : après correction du MIME, l'upload échouait avec `new row violates row-level security policy`. `generateMediaThumbnail()` utilise `upsert: true` pour autoriser la régénération au même chemin. Supabase Storage exige alors des policies **SELECT + INSERT + UPDATE** ; le bucket `medias` n'avait volontairement aucune policy SELECT pour empêcher son listing. Fix : policy `"Editors can read medias"`, limitée au bucket `medias` et à `has_min_role('editor')`. Anon et les utilisateurs ordinaires restent bloqués ; seuls editor/admin disposent du SELECT requis par l'upsert.
