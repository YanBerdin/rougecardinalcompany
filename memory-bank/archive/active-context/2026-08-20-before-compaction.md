...
2026-08-06 : ✅ Vérification production de TASK107. `https://compagnie-rouge-cardinal.fr/sitemap.xml` répond `200` sans redirection, `application/xml`, XML validé par `xmllint`; Googlebot et `Google-InspectionTool` obtiennent la même réponse. Les 9 URLs déclarées répondent toutes `200`, et `robots.txt` référence le bon endpoint. Si Search Console affiche encore « Impossible de lire le sitemap », il faut relancer la lecture dans la propriété URL exacte après propagation du déploiement; aucun défaut HTTP/XML n'est reproduit côté serveur.

2026-08-06 : 🔄 TASK107 — correction locale de la 404 `/sitemap.xml`. `app/sitemap.ts` fournit désormais une Metadata Route statique basée sur `WEBSITE_URL`, avec 9 URLs publiques canoniques et sans dépendance Supabase. `pnpm type-check` et `pnpm build` réussissent ; `next start` répond `200 OK` avec `application/xml`, un `<urlset>` valide et aucune URL privée. `robots.txt` conserve la référence correcte. La production est désormais vérifiée; seul le rafraîchissement du statut Search Console reste en attente. L'intégration de `sitemap_entries` est volontairement différée.
...
2026-08-05 : 🔄 Migration TASK200 engagée vers le correctif officiel Sharp/nft de Next.js. `next` et `eslint-config-next` sont désormais en `16.3.0`, Sharp reste en `0.35.3` et le correctif upstream référencé est `Turbopack: fix tracing of sharp@0.35` (#94845), avec `@vercel/nft` `1.10.2` embarqué. Le workaround TASK104 (`serverExternalPackages: ["sharp"]` + `outputFileTracingIncludes` vers les paquets physiques `.pnpm`) est volontairement conservé. Validations locales : installation frozen, type-check, tests image 11/11, build et manifests `.nft.json` contenant `libvips-cpp.so.8.18.3`, le binaire Sharp et `sharp/dist/index.cjs`. Reste à valider : preview/package Vercel, runtime après cold start, upload et régénération de thumbnails ; le retrait du tracing manuel devra ensuite être testé par étapes. Voir `memory-bank/tasks/TASK200-migration-correctif-upstream-sharp-nft.md`.

2026-08-03 : ✅ Remédiation CodeQL #28 (`js/xss`) sur le parcours `/auth/accept-invitation`. Le Client Component validait déjà le paramètre `url` avec `isSafeInvitationUrl()`, mais écrivait ensuite la valeur issue de la query dans `window.location.href`, un sink XSS que CodeQL ne pouvait pas relier au validateur personnalisé. Fix : nouvelle Server Action locale `continueInvitationAction()` dans `app/(marketing)/auth/accept-invitation/actions.ts`. Le bouton soumet un formulaire POST avec l'URL ; l'action traite `FormData` comme hostile, revalide HTTPS + hôte Supabase exact + chemin `/auth/v1/verify` + origine exacte de `redirect_to`, puis appelle `redirect()` seulement si la valeur est autorisée. Les URLs invalides retournent à la page intermédiaire. L'assignation client directe est supprimée. Le mécanisme anti-scanner est préservé : les scanners qui font un simple GET ne soumettent pas le formulaire, donc ne consomment pas le jeton. Validation : tests URL 5/5, `pnpm type-check`, diagnostics et `git diff --check` verts. Détail : `memory-bank/tasks/tasks-completed/TASK106-codeql-client-side-xss-invitation.md`. La fermeture GitHub dépend de la réanalyse CodeQL après push sur `master`.

2026-08-01 : ✅ Déploiement Vercel final validé après le correctif Sharp/Turbopack. Le commit `81d538f` conserve `serverExternalPackages: ["sharp"]`, trace les fichiers physiques `@img/sharp-*` sous `node_modules/.pnpm/` et supprime le hoisting Sharp de `.npmrc`. Le hoisting précédent créait des symlinks refusés par Vercel (`invalid deployment package ... files in symlinked directories`). Résultat confirmé : accès à `/admin/media/library` sans erreur 500 en production. L'incident est détaillé dans `memory-bank/tasks/tasks-completed/TASK104-dependabot-dependency-remediation.md`.

2026-08-01 : ✅ Fix erreurs 500 (100% error rate) sur `/admin/media/library` en production — root cause distincte des fix précédents du même jour (RBAC `requireMinRole`/`requireBackofficePageAccess`). Diagnostic via `mcp_sentry_mcp_search_issues` (`is:unresolved`, période 24h) : 2 issues Sentry avec culprit exact `GET /admin/media/library` — `Failed to load external module sharp-<hash>: Could not load the "sharp" module using the linux-x64 runtime` et une variante `Cannot find module 'sharp-<hash>'`. Root cause : `sharp` (utilisé pour la génération de thumbnails, `lib/dal/media-thumbnail.ts`) était bundlé par Next.js/Turbopack au lieu d'être chargé nativement via `require()` Node — le binaire natif Linux ne se charge plus une fois bundlé, cassant toute requête touchant ce chemin (`listMediaItems` en dépend indirectement). Fix : ajout de `serverExternalPackages: ["sharp"]` dans `next.config.ts`. Bonus : `app/robots.ts` créé (n'existait pas), `Disallow: /admin, /api, /auth, /debug-auth*` pour réduire les invocations de crawlers sur le backoffice ; `package.json` → `pnpm.onlyBuiltDependencies: ["@sentry/cli", "esbuild", "unrs-resolver"]` pour supprimer le warning build pnpm (scripts d'install bloqués par défaut depuis pnpm v10). Point distinct non lié au 500 (vérifié : n'affecte que les cold starts, pas les autres routes qui restent à 0% error rate) : issue Sentry séparée `Missing SUPABASE_URL or PROJECT_REF` (215 events, `lib/env-validation.ts` via `instrumentation.ts::register()`) — variable d'env `SUPABASE_PROJECT_REF` manquante sur Vercel prod, action manuelle requise côté dashboard Vercel (non corrigible par du code). Détails dans `/memories/repo/vercel-cpu-usage-and-build-warnings.md`. Déploiement Vercel requis pour validation.

2026-08-01 : ✅ Restauration des GRANTs `service_role` en production + élucidation du 500 résiduel sur `/admin/media/library`. Trois problèmes distincts traités dans la même session. (1) **GRANTs presse** : les vues `articles_presse_public` et `communiques_presse_public` avaient perdu leur `GRANT SELECT` pour `anon` (un `DROP VIEW` + `CREATE VIEW` dans `20260202200333_add_spectacle_paragraphs.sql` réinitialise les ACL, non capturées par `migra`) et la RPC `communiques_presse_dashboard()` levait `42804` (`evenement_date date` alimentée par un `timestamptz`) — migration `20260801131043_restore_press_public_view_grants_and_fix_dashboard_cast.sql` (`revoke all` + `grant select` + `e.date_debut::date`), appliquée sur staging et production. (2) **Drift massif des GRANTs `service_role` en production** : sur les 53 relations de `public`, `service_role` ne possédait plus que `Dxtm` (TRUNCATE/REFERENCES/TRIGGER/MAINTAIN) sans aucun `arwd`, et 0/28 séquences étaient utilisables ; `user_invitations` n'accordait plus de DML à `authenticated`, rendant ses policies RLS admin inatteignables. Rappel du modèle : `service_role` contourne RLS mais **reste soumis aux GRANTs table-level**, donc tout chemin `createAdminClient()` échouait en `permission denied` (confirmé dans les logs PostgreSQL : `profiles`, `partners`, `analytics_events`, `spectacles`, `evenements`, `membres_equipe_admin`, `analytics_summary`, `content_versions_detailed`…). Fix : migration `20260801134257_restore_service_role_baseline_grants.sql` + nouveau fichier déclaratif `supabase/schemas/70_grants_baseline.sql` (privilèges que `migra` ne sait pas diffuser), appliqués sur les deux projets avec la même version dans `supabase_migrations.schema_migrations`. **Aucun privilège `anon` n'a été ajouté** : `contacts_presse`, `seo_redirects` et `data_retention_recent_audit` restent volontairement plus stricts en production qu'en staging. Vérification production : 53/53 relations et 28/28 séquences pour `service_role`, `user_invitations` OK pour `authenticated`, `anon` toujours refusé sur `contacts_presse` ; `test:dal` 13/13 sur les deux environnements. (3) **500 résiduel sur `/admin/media/library`** : après la restauration des GRANTs l'erreur persistait en local. Root cause : le serveur du port 3000 était un `next start` servant un `.next` antérieur au fix `requireBackofficePageAccess()` (entrée suivante) ; en mode production Next masque la stack et renvoie un `Internal Server Error` brut, ce qui a fait croire à un bug applicatif toujours présent. Preuve : la même route rendue par un serveur `next dev` affiche la bibliothèque sans erreur, et `pnpm build` passe intégralement. Leçon : ne jamais diagnostiquer une 500 admin sur un `next start` sans avoir rebuildé — reproduire systématiquement en `next dev`. Outillage ajouté au passage : sélecteur d'environnement `SUPABASE_ENV=staging|production` (`scripts/lib/load-supabase-env.ts`, `scripts/supabase-env.sh`, `test-all-dal-functions.ts`, `diagnose-admin-views.js`) pour cibler explicitement `.env.local` (staging `yvtrlvmbofklefxcxrzv`) ou `.env` (production `hjmwctzqljfszuwkaadd`). ⚠️ `pnpm db:push` échoue en `403 LegacyDbConfigLoginRoleStatusError` (token d'accès sans privilèges) : les DDL ont été appliqués via MCP `execute_sql` suivis d'un `insert into supabase_migrations.schema_migrations`. Reste ouvert et indépendant : aucun `profiles` avec `role='admin'` en production — voir `TASK105`.

2026-08-01 : ✅ Fix 500 sur `/admin/media/library` (et 13 autres pages editor+) — `requireMinRole("editor")` utilisé comme garde de page dans `page.tsx` lève une `Error` brute (pas de `redirect()`) si le rôle est insuffisant ou si `getClaims()` échoue transitoirement (ex. `401` observé sur `GET /rest/v1/configurations_site` dans les logs Supabase, symptomatique d'un JWT/session ponctuellement invalide côté navigateur). Toute défaillance transitoire devenait donc une exception non catchée pendant le rendu du Server Component → 500 au lieu d'une redirection propre vers `/auth/login`, contrairement aux pages admin-only qui utilisent `requireAdminPageAccess()` (redirect-based). Fix : remplacement de `requireMinRole("editor")` par `requireBackofficePageAccess()` (déjà existant, redirect-based) dans les 14 fichiers `page.tsx` concernés — `agenda`, `compagnie` (+ `presentation`, `valeurs`), `lieux`, `media` (+ `folders`, `library`, `tags`), `presse`, `spectacles` (+ `[id]`, `[id]/edit`, `new`). `requireMinRole`/`requireAdminOnly` restent corrects dans la couche DAL où throw est le comportement attendu (catché par la Server Action appelante). `get_errors` : 0 erreur sur les 14 fichiers. Note capitalisée dans `/memories/repo/roles-and-permissions.md`. Cette correction n'élimine pas la cause du 401 initial (session/JWT ponctuellement invalide) mais empêche ce type d'incident transitoire de faire planter la page en 500. Aucun travail en cours.

2026-08-01 : ✅ Correction des images cassées après déplacement de média (`/_next/image` → `400 Bad Request` sur `https://hjmwctzqljfszuwkaadd.supabase.co/.../medias/uploads/1784562445305-Affiche-expo-photo---Nouveaux-Mondes.png`). Fausse piste écartée d'emblée : `next.config.ts` → `images.remotePatterns` contient bien le hostname production, contrairement à l'incident du 2026-07-20. Diagnostic réel : l'URL amont Supabase Storage répondait elle-même `{"statusCode":"404","error":"not_found","code":"NoSuchKey"}` ; `next/image` transforme cet échec de fetch amont en `400`. Root cause : le média `id = 15` a été déplacé de `uploads/` vers `photos/` le 2026-07-24 par `moveMediaItemsToFolder` (`lib/dal/media-move.ts`), qui déplace l'objet Storage et met à jour `medias.storage_path`/`thumbnail_path`/`folder_id`, mais **ne réécrit pas les URLs publiques absolues dénormalisées** stockées dans les tables consommatrices. Un audit générique via `query_to_xml` sur toutes les colonnes `text` de `public` a identifié 6 colonnes concernées (`articles_presse.image_url`, `compagnie_presentation_sections.image_url`, `home_about_content.image_url`, `home_hero_slides.image_url`, `membres_equipe.image_url`, `spectacles.image_url`) et 4 URLs mortes, dont 2 causées par ce déplacement (`spectacles#18`, `articles_presse#5`). Fix code : nouveau DAL `lib/dal/media-url-sync.ts::syncMediaReferenceUrls(previousStoragePath, nextStoragePath)` qui réécrit ces 6 colonnes en best-effort (une table en échec est loggée sans interrompre la synchronisation, l'objet Storage et l'enregistrement `medias` étant déjà cohérents) ; appelé depuis `moveSingleMedia()` après la mise à jour de `medias`, pour l'original **et** la vignette, ainsi qu'en sens inverse dans `restoreSingleMedia()` pour rester correct lors du rollback compensatoire d'un déplacement groupé. Note d'implémentation : `PostgrestFilterBuilder` est un _*thenable*_ et non une `Promise`, le type de retour des updaters doit donc être `PromiseLike<...>` sous peine d'erreur de compilation. Fix données appliqué en production sur `hjmwctzqljfszuwkaadd` : les deux `image_url` repointent vers `photos/1784562445305-Affiche-expo-photo---Nouveaux-Mondes.png` (vérifié `200`). ⚠️ Reste 2 URLs mortes hors périmètre, pointant vers l'ancien projet Supabase `yvtrlvmbofklefxcxrzv` : `compagnie_presentation_sections#7` (`team/1776350784063-IMG_3043---retouch-.jpg`, remplaçable par le média 55 `team/1784729293391-1776350784063-IMG_3043---retouch-.webp`) et `home_hero_slides#1` (`home-hero/1771866905066-hero-theatre.jpg`, aucun média correspondant → ré-upload requis). Validation : `pnpm exec tsc --noEmit` sans erreur. Le 500 sur `/admin/media/library` reste à investiguer séparément.

2026-08-01 : ✅ Correction de l'accès `/admin/media/library` après durcissement RBAC. La base de production confirme `admin` dans `auth.users.raw_app_meta_data` et `public.profiles`, mais une session JWT déjà émise pouvait ne pas contenir ce rôle. `lib/auth/roles.ts` et `supabase/middleware.ts` relisent désormais `auth.getUser()` uniquement lorsque le claim `app_metadata.role` est absent ou vaut `user`, sans utiliser `user_metadata` pour l'autorisation. Validation : `pnpm type-check`, ESLint ciblé, diagnostics VS Code et `git diff --check` réussis. Déploiement Vercel requis avant nouveau test.

2026-08-01 : ✅ Correction de l'alerte CodeQL #30 (`js/incomplete-url-substring-sanitization`) sur la page d'acceptation d'invitation. Dans `app/(marketing)/auth/accept-invitation/page.tsx`, la validation de `redirect_to` utilisait `startsWith`, ce qui acceptait des hôtes trompeurs comme `https://compagnie-rouge-cardinal.fr.evil.test`. La correction parse désormais l'URL avec `new URL(redirectTo).origin` et compare cette origine exactement à la whitelist (`window.location.origin` et `https://compagnie-rouge-cardinal.fr`). Les domaines suffixés, préfixés ou autrement différents sont ainsi rejetés. Validations : `get_errors` sans erreur, `pnpm type-check` réussi et `git diff --check` réussi. Aucun test unitaire URL dédié n'a été ajouté.

2026-08-01 : ✅ Remédiation complète des alertes Dependabot — 14 vulnérabilités initiales (9 HIGH, 5 MODERATE) corrigées. `next` passe à `16.2.11`, `sharp` à `^0.35.0`, `postcss` à `^8.5.18`; overrides PNPM renforcés pour `sharp`, `fast-uri`, `postcss`, `brace-expansion` et `js-yaml`. Le lockfile résout notamment Sharp `0.35.3` et PostCSS `8.5.25`. `lib/utils/image-compress.ts` utilise `ReturnType<typeof sharp>` pour rester compatible avec Sharp 0.35. Validations : `pnpm audit --prod`, `pnpm audit`, `pnpm install --frozen-lockfile`, `pnpm type-check`, tests image-compress 11/11 et `git diff --check` réussis. `pnpm lint` reste bloqué par la fixture indépendante `e2e/tests/auth/invite-setup/invite-setup.fixtures.ts` (`use` dans `setupAccountPage`). Voir `memory-bank/tasks/TASK104-dependabot-dependency-remediation.md`.

2026-07-24 : ✅ Correctif UI complémentaire du déplacement de médias — malgré le déplacement Storage déployé, le sélecteur de dossier du panneau de détail pouvait conserver son libellé initial. Root cause : `MediaEditForm` utilise un `Select` contrôlé par `form.watch("folder_id")`, mais `form.setValue()` était appelé sans demander la mise à jour de l'état dirty/touched ; React Hook Form ne propageait donc pas systématiquement la nouvelle valeur au contrôle. Fix : `setValue("folder_id", value, { shouldDirty: true, shouldTouch: true })`. Validation : ESLint ciblé, TypeScript et diagnostics VS Code sans erreur. A déployer sur staging et production.

2026-07-24 : ✅ Fix déplacement de dossier dans `/admin/media/library` — la sélection d'un dossier ne persistait pas : les actions unitaire et groupée ne modifiaient que `public.medias.folder_id`, alors que le dossier affiché est dérivé du préfixe de `storage_path`. Le prochain rendu restaurait donc visuellement l'ancien dossier. Fix : nouveau DAL `lib/dal/media-move.ts` qui, après `requireMinRole("editor")`, déplace l'original et sa vignette éventuelle dans le bucket `medias`, met à jour `storage_path`, `thumbnail_path` et `folder_id`, avec rollback compensatoire best-effort lors d'un déplacement groupé. `updateMediaMetadataAction` et `bulkMoveMediaAction` utilisent ce DAL commun. Diagnostic production cible confirme que `hjmwctzqljfszuwkaadd` possède les quatre policies Storage editor requises (SELECT/INSERT/UPDATE/DELETE) et aucun trigger ne modifie le dossier ; aucun changement de schema ou de policy n'est requis. Validation : `pnpm type-check`, `get_errors` et diff check sans erreur. Divergence d'historique réconciliée : le SQL de `20260722145006` était identique à la migration distante `20260722145109`; le fichier local est désormais nommé selon la version distante. Correctif à déployer sur Vercel après push du commit.

2026-07-22 : ✅ Diagnostic production newsletter/contact — la souscription newsletter est saine : `anon` a le GRANT `INSERT`, les policies RLS correspondantes existent et une insertion anonyme identique au DAL réussit. Le texte « Erreur serveur. Réessayez plus tard. » provient en réalité de `components/features/public-site/contact/actions.ts`, pas du flux newsletter (qui renvoie `Subscription failed` en cas de DAL error). Les logs PostgreSQL de production confirmaient des refus RLS sur `public.messages_contact`. Root cause : divergence entre `ContactMessageSchema` et la policy INSERT : le schéma acceptait un message de 1 à 9 caractères et tout téléphone non vide, alors que RLS exige un message de 10 à 5 000 caractères et un téléphone valide de 10 caractères minimum si renseigné. Fix : `lib/schemas/contact.ts` aligné sur ces contraintes ; le formulaire renvoie maintenant une erreur de validation précise avant l'insertion. Validation : insert anonyme conforme accepté ; message court et téléphone invalide rejetés par Zod. Le code doit être déployé sur Vercel pour atteindre la production. Aucun travail en cours.

2026-07-22 : ✅ Fix affichage photos paysage des spectacles sur le site de production — les deux cartes `LandscapePhotoCard` de `/spectacles/[slug]` ne s'affichaient pas malgré des relations `spectacles_medias` et des objets Storage présents. Root cause : les privilèges table-level `SELECT` avaient disparu sur la vue `public.spectacles_landscape_photos_public` ; l'appel anonyme PostgREST retournait `42501 permission denied for view`. Le DAL `fetchSpectacleLandscapePhotos()` masque les erreurs publiques et renvoie `[]`, supprimant silencieusement les cartes du rendu. Fix : migration manuelle `20260722145006_grant_public_spectacle_landscape_photos_view.sql`, alignée avec les GRANTs déjà déclarés dans `41_views_spectacle_photos.sql`, appliquée sur le Supabase client `hjmwctzqljfszuwkaadd`. Validation : ACL `anon` + `authenticated` rétablies, requête anonyme = 4 photos, HTML production de `/spectacles/la-farce-de-maitre-pathelin` référence de nouveau la photo. Aucun travail en cours.

2026-07-20 : ✅ Fix final Storage RLS thumbnails — après correction du self-fetch, du MIME type (`Blob image/jpeg`) et de la source de rôle JWT, l'upload échouait encore avec `new row violates row-level security policy` sur staging et production. Root cause confirmée par comparaison de `pg_policies` avec la documentation Supabase : `generateMediaThumbnail()` utilise `.upload(..., { upsert: true })` pour permettre la régénération, et un upsert Storage exige **SELECT + INSERT + UPDATE** sur `storage.objects`. Le bucket `medias` avait volontairement supprimé toute policy SELECT pour empêcher le listing, donc INSERT/UPDATE corrects ne suffisaient pas. Fix : migration `20260720150000_allow_editors_select_medias_for_storage_upsert.sql` + déclaratif `02c_storage_buckets.sql`, ajout de `"Editors can read medias"` (`for select to authenticated using (bucket_id = 'medias' and (select public.has_min_role('editor')))`). Le listing reste bloqué pour anon et les utilisateurs ordinaires ; seuls editor/admin obtiennent le SELECT requis pour l'upsert. Migration appliquée sur staging `yvtrlvmbofklefxcxrzv` et production `hjmwctzqljfszuwkaadd`, policies vérifiées dans `pg_policies`, versions MCP réalignées sur `20260720150000`. Aucun travail en cours.

2026-07-20 : ✅ Fix (suite) bouton "Générer thumbnail" — après suppression du self-fetch (entrée précédente), l'erreur devenait `Failed to upload thumbnail` en staging **et** en prod. Vérification en base : **zéro** objet `*_thumb.jpg` n'avait jamais été créé — bug préexistant, indépendant du fix précédent. Root cause confirmée via les logs Supabase Storage (`POST /object/medias/... → 400`) : `generateMediaThumbnail()` passait un `Buffer` Node brut (sortie de `sharp().toBuffer()`) directement à `supabase.storage.upload(path, buffer, { contentType: "image/jpeg" })`. En runtime Node.js (Server Action), `@supabase/storage-js` ne propage pas fiablement `contentType` pour un `Buffer` brut → la requête part avec un mime type non conforme à `allowed_mime_types` du bucket `medias` → rejet `400 invalid_mime_type`. Le pattern d'upload du fichier original (qui fonctionne) enveloppait déjà les bytes dans un `Blob` explicite (`new Blob([Uint8Array.from(buffer)], { type })`). Fix : même pattern appliqué dans `generateMediaThumbnail()` avant `.upload()`. Bonus découverte lors du diagnostic : le CLI `supabase db push --linked` cible le projet **staging** `yvtrlvmbofklefxcxrzv` (distinct de la prod `hjmwctzqljfszuwkaadd`) ; un push échoué (exit 1) avait laissé staging 3 migrations en retard (`20260715120000`, `20260715130000`, `20260720140000` — dont le fix JWT-first de `has_min_role()`/`is_admin()`). Appliquées manuellement sur staging via MCP (`mcp_supabase_apply_migration`) puis versions réalignées sur les timestamps de fichiers (`UPDATE supabase_migrations.schema_migrations`). Les deux projets sont maintenant synchronisés. Leçons capturées dans `/memories/repo/site-url-config.md`. Aucun travail en cours.

2026-07-20 : ✅ Fix bouton "Générer thumbnail" — `fetch failed` en staging et `Erreur serveur: HTTP 500` en production. Root cause : `uploadMediaImage` et `regenerateThumbnailAction` (`lib/actions/media-actions.ts`) faisaient un **self-fetch HTTP interne** (`fetch(${env.NEXT_PUBLIC_SITE_URL}/api/admin/media/thumbnail)`) vers leur propre route API, en forward manuel des cookies pour l'auth — anti-pattern Next.js (jamais appeler sa propre API route via HTTP depuis un Server Component/Server Action). `NEXT_PUBLIC_SITE_URL` non joignable depuis le serveur en staging → `fetch failed` ; en prod l'appel HTTP aboutissait à une erreur 500 côté route. Fix définitif : extraction de toute la logique de génération (download Storage → resize Sharp 300x300 JPEG 80% → upload Storage → update `medias.thumbnail_path`) dans une nouvelle fonction DAL `lib/dal/media-thumbnail.ts::generateMediaThumbnail()`, appelée **directement** (function call, zéro fetch, zéro cookie forwarding) par les deux Server Actions. La route `app/api/admin/media/thumbnail/route.ts` devient un thin wrapper autour de cette fonction, conservée seulement pour d'éventuels appels externes futurs — plus utilisée en interne. Docs mises à jour : `memory-bank/thumbnail-flow.md` (diagramme + sections composants). Leçon capitalisée dans `/memories/repo/site-url-config.md`. 0 erreur TypeScript. Aucun travail en cours.

2026-07-20 : ✅ Fix affichage images Médiathèque `/admin/media/library` (fichiers listés mais images non affichées + bouton « Générer thumbnail » → `Erreur serveur: HTTP 500`). Root cause réelle, différente du diagnostic RLS/Storage initial (voir entrée suivante) : `next.config.ts` → `images.remotePatterns` ne whitelistait que l'ancien hostname Supabase dev `yvtrlvmbofklefxcxrzv.supabase.co`, alors que le projet Supabase **production réellement lié** (`supabase/.temp/project-ref`, `.env` non commenté) est `hjmwctzqljfszuwkaadd.supabase.co`. `next/image` (utilisé par `MediaCardThumbnail.tsx`) refuse de charger toute image dont le hostname n'est pas explicitement autorisé dans `remotePatterns` — même si l'URL est publiquement accessible (vérifié : `curl` direct sur l'URL publique Storage retourne `200 OK` avec un PNG valide). Diagnostic confirmé par erreur de méthode : les tests initiaux (upload test, `curl` public URL, `.download()` service_role) avaient été exécutés par erreur contre `.env.local` (`yvtrlvmbofklefxcxrzv`, un projet Supabase différent/obsolète), donnant un faux résultat "objet manquant en Storage" — alors que le vrai projet lié (`hjmwctzqljfszuwkaadd`, confirmé via `supabase/.temp/project-ref`) contenait bien le fichier, accessible normalement. Fix appliqué dans `next.config.ts` : hostname dérivé dynamiquement de `NEXT_PUBLIC_SUPABASE_URL` (`supabaseStorageHostname = new URL(supabaseUrl).hostname`) ajouté à `remotePatterns`, plus ajout explicite de `hjmwctzqljfszuwkaadd.supabase.co` en dur (`yvtrlvmbofklefxcxrzv.supabase.co` conservé pour compatibilité dev). ⚠️ Nécessite un **redéploiement** pour prendre effet (`remotePatterns` est figé au build Next.js, non modifiable à chaud). Leçon capitalisée : toujours vérifier `supabase/.temp/project-ref` (project réellement lié aux outils MCP/CLI) avant de tester des URLs Storage via `.env.local`/`.env` — plusieurs projets Supabase coexistent dans ce repo (dev `yvtrlvmbofklefxcxrzv` vs production client `hjmwctzqljfszuwkaadd`). Aucun autre travail en cours.

2026-07-20 : ✅ Fix permissions Bibliothèque Médias (`media_tags`) et génération de thumbnails (Storage RLS). Root cause double : (1) les tables `media_tags`, `media_folders` et `media_item_tags` avaient des RLS policies mais **aucun GRANT table-level** pour `authenticated`, provoquant `permission denied for table media_tags` en production ; (2) la policy Storage INSERT du bucket `medias` exige `has_min_role('editor')`, mais `public.has_min_role()` lisait `public.profiles.role` alors que la source de vérité projet est `app_metadata.role` du JWT signé. Résultat : l’API route thumbnail autorisait l’appel (elle lit `app_metadata.role`) mais Storage refusait l’upload quand `profiles.role` était désynchronisé ou absent. Fix : migration `20260720140000_fix_media_tags_grants_and_role_functions.sql` + schéma déclaratif synchronisé (`02b_functions_core.sql`, `04_table_media_tags_folders.sql`) : GRANT SELECT/INSERT/UPDATE/DELETE sur les 3 tables media_* à `authenticated`, et réécriture JWT-first de `public.is_admin()` et `public.has_min_role(text)` avec fallback sur `profiles.role`. Voir `supabase/migrations/migrations.md` (entrée 2026-07-20). Statut : migration créée localement, à pousser sur Supabase Cloud. Aucun autre travail en cours.

2026-07-20 : ✅ Fix bouton "Continuer vers l'activation" désactivé en permanence sur la page `accept-invitation` (le bouton restait grisé indéfiniment, même avec un lien d'invitation valide). Root cause dans `app/(marketing)/auth/accept-invitation/page.tsx` : `parseInvitationFromLocation()` était passé comme **initialiseur paresseux de `useState`**. Cet initialiseur s'exécute pendant le rendu serveur (SSR), où `window` est `undefined` → il retournait `{ error: null, targetUrl: null }`. À l'hydratation côté client, React **réutilise** l'état calculé côté serveur **sans ré-exécuter** l'initialiseur — donc `targetUrl` restait `null` pour toujours et le bouton (`disabled={!targetUrl}`) ne s'activait jamais, quel que soit le lien. Fix : la lecture de `window.location.search` se fait désormais dans un `useEffect` déclenché après le montage client (`setInvitation(parseInvitationFromLocation())`), avec état initial `{ error: null, targetUrl: null }`. L'avertissement lint `react-hooks/set-state-in-effect` est explicitement supprimé sur cette ligne (`eslint-disable-next-line`) car c'est le pattern correct pour synchroniser une valeur client-only (browser API) après hydratation — cas différent d'un état dérivé qui devrait être calculé pendant le rendu. La logique anti-open-redirect (`isSafeInvitationUrl`, host Supabase + `/auth/v1/verify` + `redirect_to`) est inchangée. Validé : `get_errors` → 0 erreur/warning. Ce fix est indépendant et complémentaire du fix du même jour sur le double-décodage de l'URL (`decodeURIComponent`) documenté ci-dessous. Aucun travail en cours.

2026-07-20 : ✅ Fix bouton "Continuer vers l'activation" non cliquable sur la page `accept-invitation` (le lien d'invitation légitime était jugé invalide, affichant « Lien d'invitation invalide ou non reconnu. » au lieu du bouton actif). Root cause dans `app/(marketing)/auth/accept-invitation/page.tsx` : (1) double décodage de l'URL — `URLSearchParams.get("url")` décode déjà le percent-encoding une fois automatiquement, mais le code appelait ensuite `decodeURIComponent()` une seconde fois sur la valeur déjà décodée, alors que le lien n'est encodé qu'une seule fois côté serveur (`encodeURIComponent` dans `lib/email/actions.ts::sendInvitationEmail`) ; (2) la vérification anti-open-redirect `redirectTo.startsWith(window.location.origin)` était trop stricte et pouvait échouer selon l'hôte réel (www vs apex, proxy). Fix : suppression du second `decodeURIComponent` (utilisation directe de la valeur décodée par `URLSearchParams`), ajout d'une constante `PRODUCTION_ORIGIN = "https://compagnie-rouge-cardinal.fr"` utilisée en fallback aux côtés de `window.location.origin` dans `isSafeInvitationUrl`. La logique anti-open-redirect (host Supabase + path `/auth/v1/verify` + `redirect_to`) reste inchangée sur le fond. Validé par test live (fetch de la page en prod) : avant correctif la page affichait l'erreur "invalide" au lieu du bouton actif. 0 erreur TypeScript. Leçon capitalisée dans `/memories/repo/site-url-config.md`. ⚠️ Renvoyer une nouvelle invitation après déploiement (jeton précédent potentiellement expiré). Aucun travail en cours.

2026-07-20 : ✅ Fix page d'invitation `accept-invitation` plantant côté client (« Une erreur est survenue » + console : « ❌ Attempted to access a server-side environment variable on the client »). Root cause : `app/(marketing)/auth/accept-invitation/page.tsx` est un **Client Component** (`"use client"`) mais importait `@/lib/env` (t3-env) et `@/lib/site-config` — ce dernier importe lui-même `@/lib/env` et lit des vars server-only (`EMAIL_FROM`, `EMAIL_CONTACT`). Ces imports embarquent la validation t3-env server dans le bundle client, qui crash à l'accès de `NEXT_PUBLIC_SUPABASE_URL` → la page d'activation affichait l'erreur générique et le jeton OTP n'était jamais consommé. C'est la même exception documentée que `supabase/client.ts`. Fix : `env.NEXT_PUBLIC_SUPABASE_URL` → `process.env.NEXT_PUBLIC_SUPABASE_URL!` (var `NEXT_PUBLIC_*` injectée au build, client-safe) ; `WEBSITE_URL` → `window.location.origin` pour la validation `redirect_to.startsWith(...)` (la page est servie depuis l'URL du site, code exécuté uniquement après le guard `typeof window`) ; `SITE_CONFIG.SEO.TITLE` → constante locale `SITE_TITLE = "Rouge Cardinal"`. La logique anti-open-redirect (host Supabase + path `/auth/v1/verify` + `redirect_to`) est préservée. 0 erreur TypeScript. ⚠️ Action requise après déploiement : renvoyer l'invitation (le lien précédent a pu consommer/expirer son jeton). Leçon capitalisée dans `/memories/repo/site-url-config.md` : ne JAMAIS importer `@/lib/env` ou `@/lib/site-config` dans un Client Component. Aucun travail en cours.

2026-07-19 : ✅ Fix invitations admin expirant instantanément (`otp_expired`) — Root cause : le lien Supabase brut (`generateLink` invite, jeton OTP à usage unique) était envoyé tel quel dans l'email. Les passerelles mail d'entreprise / antivirus (Outlook Safe Links, Proofpoint, Mimecast) préchargent automatiquement (GET) tous les liens d'un email dès sa réception pour les scanner contre le phishing — cela consomme silencieusement le jeton à usage unique avant même que le vrai destinataire ne clique, provoquant `#error=access_denied&error_code=otp_expired` en quelques minutes. Fix : nouvelle page intermédiaire `app/(marketing)/auth/accept-invitation/page.tsx` qui exige un vrai clic JS (`onClick`) avant de naviguer vers le lien Supabase réel — les scanners automatisés (simple GET, pas d'exécution JS) ne déclenchent jamais ce clic, donc le jeton survit jusqu'à l'action réelle de l'utilisateur. `lib/email/actions.ts::sendInvitationEmail()` enveloppe désormais le lien réel derrière `${WEBSITE_URL}/auth/accept-invitation?url=<lien encodé>`. La page de confirmation valide que l'URL décodée pointe vers l'hôte Supabase (`NEXT_PUBLIC_SUPABASE_URL`), que le path commence par `/auth/v1/verify`, et que `redirect_to` pointe vers `WEBSITE_URL` (protection anti-open-redirect). Bonus fix UX : `app/(marketing)/auth/setup-account/page.tsx` ne détectait que `hash.includes('access_token')` et affichait le message générique trompeur "Veuillez utiliser le lien d'invitation reçu par email" même quand Supabase redirigeait avec un hash d'erreur explicite (`#error=access_denied&error_code=otp_expired`) — ajout d'une détection prioritaire du hash d'erreur avec message actionnable ("lien expiré ou déjà utilisé, demandez une nouvelle invitation"). Bonus lint fix : la page `accept-invitation` utilise une initialisation paresseuse `useState(parseInvitationFromLocation)` au lieu de `useEffect + setState` pour éviter l'avertissement React "cascading renders" (state calculé une seule fois au montage à partir de `window.location`, sans besoin d'effet). ⚠️ Action requise : les invitations envoyées AVANT ce fix contiennent des liens Supabase bruts et échoueront toujours — elles doivent être renvoyées après déploiement. Recommandé également : vérifier dans le Dashboard Supabase (Authentication → Emails) la valeur "Email OTP Expiration". Aucun travail en cours.

2026-07-16 : ✅ Fix erreur build Vercel `DYNAMIC_SERVER_USAGE` avalée sur `/spectacles` (ISR `revalidate=60`). Root cause : `fetchAllSpectacles()` et les autres fonctions de lecture publiques de `lib/dal/spectacles.ts` / `lib/dal/spectacle-photos.ts` appellent `createClient()` (`supabase/server.ts`) qui utilise `cookies()`. Lors de la tentative de rendu statique au build (page ISR), Next.js lève une erreur de contrôle interne `DYNAMIC_SERVER_USAGE` — mais le `catch` générique de ces fonctions l'avalait (log `console.error` + retour `[]`/`null`), empêchant Next.js de marquer correctement la route comme dynamique et polluant les logs de build avec une fausse erreur. Fix : nouveau helper `isDynamicServerError()` (`lib/dal/helpers/error.ts`, exporté via `lib/dal/helpers/index.ts`) qui détecte `err.digest === "DYNAMIC_SERVER_USAGE"` ; ajout d'un `if (isDynamicServerError(err)) throw err;` en tête de chaque `catch` des fonctions de lecture publiques concernées : `fetchAllSpectacles`, `fetchSpectacleById`, `fetchSpectacleBySlug`, `fetchSpectacleNextVenue`, `fetchSpectacleTicketUrl`, `fetchSpectacleTicketInfo`, `fetchTicketUrlsForSpectacles`, `fetchEventDateRangesForSpectacles`, `fetchDistinctGenres` (`lib/dal/spectacles.ts`) et `fetchSpectacleLandscapePhotos`, `fetchSpectacleGalleryPhotos` (`lib/dal/spectacle-photos.ts`). Conforme à la recommandation officielle Next.js (https://nextjs.org/docs/messages/dynamic-server-error) : ne jamais avaler cette erreur de contrôle de flux, toujours la relancer pour laisser Next.js gérer le fallback dynamique. Aucun travail en cours. — GRANT DELETE manquant sur `public.profiles` pour `authenticated`. Signalement utilisateur : un compte enregistré par erreur (`florian.chaillot@outlook.fr`) n'apparaissait plus dans `public.profiles` mais restait visible dans le dashboard admin (`/admin/users`) avec le statut « Vérifié » — la liste admin est construite depuis `auth.users` via `listAllUsers()`, pas depuis `profiles`. La suppression échouait avec `permission denied for table profiles`. Root cause : `60_rls_profiles.sql` définit bien une policy RLS `DELETE` (self OU `is_admin()`), mais `20251027020000_restore_basic_grants_for_rls.sql` n'avait restauré que `SELECT, INSERT, UPDATE` sur `profiles` après la campagne de sécurité — le `GRANT DELETE` avait été oublié, bloquant l'opération avant même l'évaluation de la policy RLS. Fix : migration `20260715130000_grant_delete_profiles_to_authenticated.sql` (`grant delete on table public.profiles to authenticated`) + déclaratif synchronisé (`02_table_profiles.sql`) + `lib/dal/admin-users.ts:deleteUser()` rendu résilient (un échec de suppression du profil ne bloque plus la suppression de `auth.users`, pour éviter un compte fantôme si le même problème de GRANT réapparaît). Appliquée via `supabase db push` sur le projet Supabase Cloud du client (`hjmwctzqljfszuwkaadd`) — profil fantôme supprimé avec succès. **Confirmation rôles** : seuls les admins peuvent supprimer un utilisateur — double barrière (défense en profondeur) : (1) page `/admin/users` protégée par `requireAdminPageAccess()`, (2) `deleteUser()` (DAL) protégé par `requireAdminOnly()` avant même d'atteindre la RLS. Un `editor` ou un `user` n'a accès ni à la page ni à la Server Action ; la policy RLS elle-même n'autorise que le self-delete ou `is_admin()`. Voir `supabase/migrations/migrations.md` (entrée 2026-07-15). Aucun travail en cours.

2026-07-14 : ✅ Suppression de 7 tables de liaison jamais exploitées (`spectacles_membres_equipe`, `articles_medias`, `spectacles_categories`, `spectacles_tags`, `articles_categories`, `articles_tags`, `pending_invitations`) — migration `20260713120000_drop_unused_leaf_tables.sql`. Suite à un audit (`doc-perso/Mai-2026/00-unused-tables.md`) listant des tables vides suspectées inutilisées, vérification croisée code runtime + requêtes MCP Supabase sur le cloud (`yvtrlvmbofklefxcxrzv`) : type d'objet (table/vue), lignes réelles, FK entrantes, vues dépendantes. Correction du périmètre initial : `categories` (9 lignes), `tags` (15), `communiques_categories` (4), `communiques_tags`/`communiques_medias` (couplées à la vue `communiques_presse_public`) et `media_item_tags` (2 lignes, utilisée par `lib/dal/media.ts`) étaient à tort classées vides/inutilisées — **conservées**. `seo_redirects`/`sitemap_entries` retirées du lot puis restaurées sur demande (branchement SEO futur). Schéma déclaratif synchronisé (6 fichiers), tests `__tests__/dal/permissions-integration.test.ts` (-7 blocs) et `scripts/test-permissions-rls.ts` (-3 blocs) nettoyés, `supabase/schemas/README.md` recompté (36→29 tables RLS). Appliquée en local (`supabase db reset`), types régénérés. ⏸ Pas encore poussée sur Supabase Cloud. Voir `supabase/migrations/migrations.md` (entrée 2026-07-14). Aucun travail en cours.

2026-07-11 : ✅ Fix menu mobile header (ouverture hors du header + contraste + fermeture au clic extérieur) + type presse home — `components/layout/header.tsx` restructuré : backdrop + panneau de navigation mobile déplacés en frères `fixed` de `<header>` (fragment `<>...</>`) au lieu d'être imbriqués dans `<nav>`. Corrige deux bugs : (1) le panneau s'ouvrait compressé dans le container `max-w-screen-xl` du `<nav>` au lieu de s'afficher en pleine largeur sous le header ; (2) survol du hero en dessous du panneau ouvert faisait scintiller/trembler le flou du header — cause racine : `.header-transparent:hover` se déclenchait dès que la souris survolait le backdrop/panneau car `:hover` remonte le DOM à tous les ancêtres même quand l'élément survolé (position `fixed`) déborde visuellement de la boîte de l'ancêtre (48px header mobile vs backdrop couvrant tout le viewport) — sortir ces éléments du DOM de `<header>` élimine la propagation. Fond `.liquid-glass-mobile` passé de `rgba(255,255,255,0.08)` à `hsl(var(--popover) / 0.98)` (light + dark, `app/globals.css`) pour rester lisible sur l'image hero. Fermeture au clic extérieur + touche Échap ajoutée via listener `mousedown`/`touchstart`/`keydown`, en excluant explicitement les clics dans `[role="menu"]`/`[data-radix-popper-content-wrapper]` (portail Radix du `ThemeSwitcher`) pour ne pas fermer le panneau lors du changement de thème. Validé en direct via Playwright : bg dark mode `rgb(42,42,42,0.98)` correct, `header.matches(':hover')` reste `false` quand on survole le hero sous le panneau ouvert. Bonus indépendant : `components/features/public-site/home/news/*` — remplacement du champ `category` (hardcodé `"Presse"`) par le vrai champ `type` de `articles_presse`, aligné avec `RevueDePresse.tsx` (`lib/dal/home-news.ts` select + `FeaturedArticleRecord`, `types.ts`, `NewsContainer.tsx`, `NewsView.tsx`, `NewsCard.tsx`). Aucun travail en cours.

2026-07-04 : ✅ TASK102 — Tarif réduit (`price_reduced_cents`) + affichage public prix & capacité. Colonne `price_reduced_cents integer null` ajoutée à `evenements` (indépendante de `price_cents`, sans contrainte croisée), propagée dans toute la chaîne admin (schémas Zod `admin-agenda.ts`/`admin-agenda-ui.ts`, `EventClientDTO` dupliqué, DAL `admin-agenda.ts` 4 selects/mappings, Server Actions `actions.ts`, formulaire `EventFormFields.tsx` avec nouveau champ « Tarif réduit (€) » + renommage « Plein tarif (€) », détail `EventDetail.tsx`, pages `[id]`/`[id]/edit`) et exposée publiquement sur `/agenda` (`AgendaEventList.tsx`, helper `formatPrice`/`buildPricingSummary`, icônes `Tag`+`Users`) et `/spectacles/[slug]` (nouvelle fn DAL `fetchSpectacleTicketInfo`, prop `ticketInfo` sur `SpectacleDetailView`). `restore_content_version` (versioning) mis à jour pour restaurer le nouveau champ. Incident migration : `supabase db diff` générait un fichier de 1242 lignes bruité (drift pré-existant revoke/grant + drop/create vues/triggers sans rapport) → migration manuelle chirurgicale écrite à la main (`20260704130737_add_price_reduced_cents_evenements.sql`), conforme au style des migrations précédentes. Incident runtime : erreur "column does not exist" en dev car `pnpm dev` pointe vers Supabase Cloud (`.env.local`), pas l'instance locale — résolu par `pnpm db:push`. Validation : `pnpm tsc --noEmit`, `pnpm eslint`, `pnpm build`, `scripts/test-admin-agenda-crud.ts` tous verts. Voir `memory-bank/tasks/TASK102-reducedPriceAndPublicCapacity.md`. Aucun travail en cours.

2026-07-03 : ✅ TASK101 — Drag-and-drop réorganisation articles de presse (8 phases). Colonne `display_order` ajoutée à `articles_presse` avec backfill basé sur `row_number()` pour préserver l'ordre existant. Implémentation complète : (1) `reorderArticlesAction` Server Action (validation Zod, DAL call, revalidation 3 routes), (2) `useArticlesDnd` hook custom (sensors PointerSensor 8px + KeyboardSensor, handleDragEnd avec optimistic update et toast), (3) `SortableArticleCard` composant unique (pas de duplication mobile/desktop pour dnd-kit), (4) `ArticlesView` intégrée DndContext + closestCenter + verticalListSortingStrategy, (5-6) 2 migrations applicables (DDL + index + DML backfill), local + remote ✅. Sort order changé `published_at DESC` → `display_order ASC` sur 3 contextes (admin, page `/presse` publique, widget homepage "À la une"). 17 fichiers staged. Commit `3f9caba` feat(presse-admin). TypeScript validation = 0 erreurs. Aucun travail en cours.

2026-06-30 : ✅ TASK100 — ContactInfoSidebar dynamique. Les coordonnées sur la page `/contact` sont désormais lues depuis la DB (`public.configurations_site` clé `public:footer:content`) via `fetchFooterConfig()`. Pattern composition : `ContactServerGate` (Server Component) fait `Promise.all([fetchDisplayToggle, fetchFooterConfig])`, assemble `<ContactInfoSidebar contactInfo={contact}>{showNewsletter && <NewsletterCard />}</ContactInfoSidebar>` et le passe comme slot `sidebar: React.ReactNode` à `ContactPageView`. `ContactInfoSidebar` reçoit `contactInfo: FooterConfigDTO["contact"]` + `children?: React.ReactNode` (suppression du boolean prop `showNewsletter`). Phone conditionnel (masqué si vide). `ContactPageView` (Client Component) découplé de `FooterConfigDTO`. Fallback silencieux via `FOOTER_DEFAULTS`. Labels admin `"Footer - Pied de page"` → `"Pied de page & Coordonnées"` dans `AdminSidebar` + `CardsDashboard`. 5 fichiers modifiés, 0 erreur lint/TS. Aucun travail en cours.

2026-06-12 : ✅ Installation et configuration de Graphify (cartographie sémantique du codebase). Outil `graphifyy 0.8.37` installé dans `.graphify-venv/` (Python 3.12, `pip install "graphifyy[gemini]"`). Backend Gemini via AI Studio (`GOOGLE_API_KEY=AIzaSy...` dans `.env.local`). Extraction complète : 4 229 nœuds, 9 337 arêtes, 438 communautés nommées (0 placeholder). `.graphifyignore` créé pour exclure `.github/`, `doc/`, `memory-bank/` (−21 % de nœuds parasites). `graph.html` (3,8 Mo) généré et accessible via `python -m http.server 8765 --directory graphify-out`. Graphify intégré dans `.github/copilot-instructions.md` comme source de vérité. Modèles optimaux : `gemini-3.1-flash-lite` + `GRAPHIFY_MAX_OUTPUT_TOKENS=8192`. Procédure complète dans `memory-bank/procedures/graphify-codebase-graph.md`. Aucun travail en cours.

2026-06-03 : ✅ TASK097 — Hero Slide Video Background. Ajout de `video_url text nullable` à la table `home_hero_slides` et propagation complète sur 9 fichiers. (1) DB : colonne `video_url text` dans `supabase/schemas/07d_table_home_hero.sql` + migration `20260603120000_add_video_url_to_home_hero_slides.sql` (`ALTER TABLE … ADD COLUMN IF NOT EXISTS`). (2) Zod : `video_url: relativeOrAbsoluteUrl.optional().or(z.literal(""))` dans `HeroSlideInputSchema` + `HeroSlideFormSchema` + interface `HeroSlideDTO`, avec `.refine()` croisé image/vidéo (l'un ou l'autre requis). (3) DAL public : types `SupabaseHeroRow` + `HomeHeroSlideRecord` étendus, `video_url` ajouté au select explicite et mappé dans `rowToDTO`. (4) HeroContainer : `video: r.video_url ?? undefined` — la vue publique (`HeroSlideBackground.tsx`) rendu `<video>` existant non modifié. (5) Admin hooks : `video_url: ""` dans `DEFAULT_FORM_VALUES` (`useHeroSlideForm`) et `video_url: slide.video_url ?? ""` dans `mapSlideToFormValues` (`useHeroSlideFormSync`). (6) Admin form : composant `VideoUrlField` (Input `type="url"`, label "URL vidéo (optionnel)", description chemin relatif/absolu) dans `HeroSlideFormFields.tsx`. (7) Admin preview : Badge `<Vidéo>` conditionnel sur `slide.video_url` dans `HeroSlidePreview.tsx`. Aucun travail en cours.

2026-05-20 : ✅ TASK096 — Durcissement sécurité auth (rôle app_metadata + mot de passe + Server Action setup). Migration du rôle de `user_metadata` (modifiable par l'utilisateur — faille d'élévation de privilège) vers `app_metadata` (server-only) dans toutes les couches : 3 migrations SQL (`20260520134210`, `20260520153000`, `20260520160650`), DAL `admin-users.ts`, triggers `handle_new_user`+`handle_user_update`, guards `lib/auth/roles.ts`, 9 scripts nettoyés. `PasswordSchema` (min 12 + 4 classes de caractères) dans `lib/schemas/auth.ts`, intégré dans les 3 formulaires. `setupAccountAction` Server Action (`lib/actions/auth-setup-actions.ts`) remplace l'appel `supabase.auth.updateUser` client-side dans `SetupAccountForm`. `lib/auth/is-admin.ts` @deprecated supprimé. 29 tests unitaires verts + 4 E2E invite-setup (INVITE-SETUP-001→004) + ROLE-ESC-001 (preuve qu'un editor ne peut PAS s'élever en admin). Script `check-role-invariant.ts` + workflow CI cron quotidien `.github/workflows/check-role-invariant.yml`. **Action manuelle requise** : ajouter le secret `INVARIANT_DB_URL` dans GitHub Actions secrets. HIBP (leaked password) indisponible sur Free plan — à activer au passage Pro. Aucun travail en cours.

2026-05-17 : ✅ TASK095 — Footer administrable. Description + contact (email/téléphone/adresse) + réseaux sociaux (Facebook/Instagram/Twitter) éditables depuis `/admin/footer`. Source de vérité : ligne unique `public.configurations_site` clé `public:footer:content` (jsonb), value contient uniquement du contenu public (zéro secret). Schéma Zod `lib/schemas/footer-config.ts` (`FooterConfigInputSchema`, `FOOTER_DEFAULTS` matche le footer hardcodé pour fallback robuste). DAL `lib/dal/footer-config.ts` avec `cache()` + `.maybeSingle()` + fallback. Server Action `updateFooterConfigAction` (`parseAsync()`, `ActionResult<void>` BigInt-Three-Layer compliant, `revalidatePath('/', 'layout')` + `revalidatePath('/admin/footer')`). Footer public refactorisé en Server Component async. Page admin protégée par `requireAdminPageAccess()`. Composants admin compound (`types.ts`/Container/View/Form) dans `components/features/admin/footer/`. Entrée sidebar avec icône `LayoutTemplate`. Migration seed idempotente `20260517212052_seed_footer_config.sql` (`INSERT ... ON CONFLICT (key) DO NOTHING`) — pas encore poussée sur prod. 13 tests Vitest verts (525ms), `pnpm lint` ✅, `pnpm build` 15.7s ✅ (Next.js 16.2.6 Turbopack). RLS prod vérifiées via Supabase MCP : `Anon can view public site configurations` (anon, `key ~~ 'public:%' OR key ~~ 'display_toggle_%'`) + `Admins can update site configurations` (authenticated, `is_admin()` USING + WITH CHECK). Aucun travail en cours.

2026-05-16 : ✅ Audit sécurité Dependabot (3 patches) — (1) Upgrade Next.js 16.2.5 → 16.2.6 (contournement middleware, alerte #74, commit `c5238b5`). (2) `pnpm.overrides` `postcss >=8.5.10` (GHSA-952p-6rrq-rcjv XSS, alerte #52, commit `fb71427`). (3) `pnpm.overrides` `fast-uri >=3.1.2` (GHSA-mwcw-c2x4-8c55 path traversal + GHSA-rm9h-fv28-wqch host confusion, 2 HIGH, commit `b808a3c`). `CVE-2025-69873` (ajv ReDoS) ajouté à `pnpm.auditConfig.ignoreCves` — toutes les instances ajv (6.14.0 et 8.18.0) sont déjà aux versions patchées. `pnpm audit` local = 0 vulnérabilité. Les 2 alertes Dependabot affichées lors du push (`c5238b5`) correspondent à un lag de retraitement — non représentatives de l'état réel. Aucun travail en cours.

2026-05-15 : ✅ TASK094 — Auto-save générique articles de presse + spectacles — `useFormAutosave<TFormValues, TPayload, TUpdatePayload>` créé (`lib/hooks/use-form-autosave.ts`, 291L) : state machine `idle|dirty|saving|saved|error`, debounce 2s, heartbeat 30s, concurrence sécurisée. `AutoSaveIndicator` déplacé de `presse/` vers `admin/shared/`. `use-press-release-autosave` refactorisé en wrapper léger. Articles de presse : `createArticleAction` → `Promise<ActionResult<{ id: string }>>`, `ArticleNewForm` + `ArticleEditForm` branchés, bannière si `published_at IS NOT NULL`. Spectacles : `SpectacleForm` branché, force `public: false` + `status: "draft"`, bannière si `public === true`. 12 fichiers, +765/−270. Invariant : jamais d'auto-save sur entité publiée. Aucun travail en cours.

2026-05-15 : ✅ TASK093 — Auto-save brouillon communiqués de presse (création + édition) implémenté et aligné avec le comportement réel. `createPressReleaseAction` retourne `{ id: string }`, hook `use-press-release-autosave` en place (`idle|dirty|saving|saved|error`, debounce 2s, heartbeat 30s, concurrence sécurisée), `public: false` forcé sur les payloads auto-save, NewForm avec `savedDraftId` + `beforeunload`, EditForm auto-save désactivé pour `release.public` avec bannière de sauvegarde manuelle. Correctif post-implémentation appliqué : description autosavée même sans titre (`hasDraftContent` sans garde `title`) + typage update dédié `PressReleaseAutoSaveUpdatePayload` (`title?`) pour compatibilité TypeScript stricte. Aucun travail en cours.

2026-05-08 : ✅ TASK092 — Fix duplication ville sur `/agenda` + helper Google Maps text-first. Bug initial : `Theatre Maurice Novarina, 74200 THONON-LES-BAINS, 74200 THONON-LES-BAINS`. Deux causes cumulées : (1) `lib/dal/agenda.ts` `buildAddress` concaténait toujours `code_postal + ville` à `lieux.adresse` même quand l'adresse contient déjà la ville (cas Thonon) ; (2) `components/features/public-site/agenda/AgendaEventList.tsx` `extractPostalCity` regex gourmande `(\d{5})\s+(.+)` capturait toute la chaîne après le premier code postal. Fix : DAL skip city append si déjà présente, regex non-gourmande `([^,]+)`. En parallèle, refactor `lib/utils/google-maps.ts` : stratégie text-first (nom + adresse + ville) car `query=lat,lng` dépose un pin sans fiche établissement, alors que la requête textuelle résout vers la fiche canonique Google. 7/7 tests Vitest, lint clean, validation `curl /agenda` confirmée. Voir `memory-bank/tasks/tasks-completed/TASK092-fix-duplicate-city-agenda.md`. Aucun travail en cours.

2026-05-07 : ✅ TASK091 — Refactoring calendrier agenda + fix violations ESLint (phases A→C4). Implémentation `plan-agenda-calendar.prompt.md` : CalendarNav avec `useCallback` pour `handlePrevMonth`/`handleNextMonth` + `setSelectedDate(null)` à la navigation ; CalendarMonth avec extraction `CalendarMonthHeader`, helper `filterDayEvents`, typage `Event[]` ; CalendarWeek avec extraction `CalendarWeekHeader` ; AgendaEventList avec extraction `SelectedDateBanner` + `EventListEmptyState` ; AgendaEventCard avec extraction `resolveDisplayDate` + séparation layout mobile/desktop. Nouveaux composants : `CalendarDay.tsx`, `CalendarDayCell.tsx`, `CalendarEventChip.tsx`. Implémentation `plan-agendaViolationsFixes.prompt.md` phases A→C4 : suppression inline styles, `type="button"`, fix guillemets. Fix ESLint global : `animations.jsx` `useRef(0)` (remplace `useRef(performance.now())` impure), 4 fixtures e2e Playwright (`use` → `register`, règle `rules-of-hooks`). `pnpm lint` = 0 erreurs. Aucun travail en cours.

2026-05-02 : ✅ Fix 35 alertes Supabase Security Advisor — 4 migrations appliquées cloud + local (20260502120000/120100/120200/140000) : REVOKE FROM PUBLIC sur fonctions SECURITY DEFINER, 5 fonctions converties en SECURITY INVOKER, 6 index FK créés (communiques_*), ~18 index inutilisés supprimés. `get_audit_logs_with_email` EXECUTE révoqué de authenticated/anon, DAL migré vers `createAdminClient()`. Déclaratif synchronisé : `42_rpc_audit_logs.sql`. Action manuelle restante : `auth_leaked_password_protection` (Dashboard → Auth → Settings). Rapport : `doc/rapport-session-fix-35-advisors-2026-05-02.md`. Aucun travail en cours.

2026-05-01 : ✅ Fix démarrage Supabase local — Cache `.temp/` obsolète (storage v1.31.1 + migration `fix-optimized-search-function` introuvable) provoquait crash boucle du conteneur storage. Solution : `supabase stop --no-backup` + `rm -f` ciblé sur 6 fichiers cache de versions (`storage-migration`, `storage-version`, `cli-latest`, `gotrue-version`, `postgres-version`, `rest-version`) + `supabase start`. Stack 12 conteneurs healthy, storage v1.54.1, 200+ migrations appliquées. Procédure documentée dans `memory-bank/changes/2026-05-01-fix-supabase-local-storage-cache.md`. Recommandation forte : créer `supabase/config.toml` pour épingler les versions Docker. Aucun travail en cours.
