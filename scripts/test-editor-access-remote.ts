#!/usr/bin/env tsx
/**
 * Test RLS access as an editor-role user — REMOTE (Cloud) version.
 *
 * Verifies that:
 ...
 *
 * @usage
 *   pnpm exec tsx scripts/test-editor-access-remote.ts
 *
 * @requires
 * - .env.local with NEXT_PUBLIC_SUPABASE_URL, NEXT_PUBLIC_SUPABASE_PUBLISHABLE_OR_ANON_KEY, SUPABASE_SECRET_KEY
 */
...
if (!SUPABASE_URL || !PUBLISHABLE_KEY || !SERVICE_KEY) {
    ...
    console.error("\n📝 Ensure .env.local contains the cloud Supabase credentials.");
    process.exit(1);
}

// Safety: block execution against local Supabase
if (SUPABASE_URL.includes("127.0.0.1") || SUPABASE_URL.includes("localhost")) {
    console.error("❌ This script targets REMOTE Supabase only.");
    console.error("   Use test-editor-access-local.ts for local testing.");
    process.exit(1);
}
...
async function testCrud(client: any, check: CrudCheck): Promise<void> {
...
async function main(): Promise<void> {
    console.log("\n⚠️  Testing against REMOTE Supabase (Cloud)\n");
